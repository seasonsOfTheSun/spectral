...
for i,v in df["kl"].iteritems():
    evector,gene = i
    if not np.isnan(v):
        G.add_edge(evector, gene, weight=np.exp(-v**2))

pos = sphere.spherical_spring_layout(G)
pos_df = pd.DataFrame(pos).T

for i in range(3):
    pos_df[i] = (pos_df[i] - (-1.01))/2.1

e_or_g = pos_df.index.map(lambda x: type(x)==np.int)
pos_df.index = [("Eigenvector-"+str(i) if type(i) is int else i) for i in pos_df.index]


pos_df["type"] = pos_df.index #np.where(e_or_g, "Eigenvector", "Gene")
...
link_df = pd.DataFrame(G.edges()).applymap(lambda i:("Eigenvector-"+str(i) if type(i) is int else i)).applymap(convert_label.get)
link_df.columns=["source", "target"]

link_df["r"] = 255#np.where
link_df["g"] = 0#np.where
link_df["b"] = 0#np.where
link_df["alpha"] = 1+int(254*np.random.rand())#np.where
